...
public class View {
    ...
    private static void help() {
        ...
        System.out.println("   -count <n>          Only display (or dump) the first n images");
    ...
    public static void main(String[] args) {
        ...
        // Read and filter the set of images to display 
        ...
        if (count > 0) {
            images = Arrays.copyOf(images, count);
        }

        if (images.length == 0) {
            System.err.println("No images to view");
            return;
        }

        // View (or dump) the images
        ...
        if (dump) {
            ...
            attributes.showClassify(classify);
            attributes.showLabels(!hide);
            attributes.scale(scale);
            attributes.sleep(sleep);
            Viewer.invoke(images, title, attributes);
        }

        // Write the labels file (classify only)

        if (classify && outputFileName.length() > 0) {
            try {
                Image.writeLabels(images, outputFileName);
            } catch (IOException e) {
                System.err.println("Could not write label file: " + e.getMessage());
            }
        }
    }
}
