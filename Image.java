...
public class Image {
    ...
    private static ImageFileHeader readImageFileHeader(BufferedInputStream input) throws IOException {
        ...
        if (header.magic != ImageFileHeader.MAGIC) {
            throw new FileFormatException("Bad magic (images): " + header.magic);
        } else if (header.count <= 0) {
            throw new FileFormatException("Invalid image count: " + header.count);
        } else if (header.rows <= 0) {
            throw new FileFormatException("Invalid row size: " + header.rows);
        } else if (header.cols <= 0) {
            throw new FileFormatException("Invalid columns size: " + header.cols);
        }
        return header;
    }
    ...
    private static LabelFileHeader readLabelFileHeader(BufferedInputStream input) throws IOException {
        ...
        if (header.magic != LabelFileHeader.MAGIC) {
            throw new FileFormatException("Bad magic (labels): " + header.magic);
        } else if (header.count < 0) {
            throw new FileFormatException("Invalid label count: " + header.count);
        }
        return header;
    }
